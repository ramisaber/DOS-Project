...
def implement_consistency():
  print("synchronizing")
  r = requests.get("http://127.0.0.1:5010/return/ord1")
  if(r.text=="Sync"):
    return "Sync"
  else:
...
def buy():
    ...
    if(r.text!="No Such Name"):
        ...
        if(x[0]["qty"]<qty):
            ...
            try:
                r = requests.post("http://127.0.0.1:5001/add_internal",json = {'qty':qty,'name':name,'result':False,'reason':"Not Enough Books In The Store"})
            except:
                pass
            returned_object = {'qty':qty,'name':name,'result':False,'reason':"Not Enough Books In The Store"}
            if(r==None):
                r = requests.post("http://127.0.0.1:5010/Addrecord",json = {'source':'ord2','content':str(returned_object)})
            return Response(
            ...
            try:
                r = requests.post("http://127.0.0.1:5001/add_internal",json = {'qty':qty,'name':name,'result':True,'reason':"Success"})
            except:
                pass
        ...
        r=None
        try:
            r = requests.post("http://127.0.0.1:5001/add_internal",json = {'qty':qty,'name':name,'result':False,'reason':"No Such book exist"})
        except:
            pass
