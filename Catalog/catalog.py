...
def implement_consistency():
  print("synchronizing")
  r = requests.get("http://127.0.0.1:5010/return/cat2")
  if(r.text=="Sync"):
    return "Sync"
  else:
...
def update_product(name):
  ...
  try:
    r = requests.put("http://127.0.0.1:5002/update-internal/"+name,json = {'qty':qty,'price':price})
  except:
    pass
  returned_object={'qty':qty,'price':price,'name':name}
  #print (str(returned_object))
  if(r==None):
    r = requests.post("http://127.0.0.1:5010/Addrecord",json = {'source':'cat1','content':str(returned_object)})
